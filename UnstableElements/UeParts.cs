	...
	public static void AddPartTypes(){
		...
		QApi.AddPartType(Sublimation, (part, pos, editor, renderer) => {
			...
			if(myState.field_2743){
				...
				progress = editor.method_504();
			}

			if(progress < 0.5){ // render under irises
				Editor.method_925(stabilizedAether, RelativeToGlobal(renderInfo, new(0, 0)), new(0, 0), renderInfo.field_1985, 1f, progress, 1f, false, null);
				Editor.method_925(stbAetherRot, RelativeToGlobal(renderInfo, new(0, 0)), new(0, 0), renderInfo.field_1985, 1f, progress, 1f, false, null);
			}
